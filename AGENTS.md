...
| 优先级 | 文件 | 问题 | 预计工作量 |
|--------|------|------|-----------|
| P1 | `report_updater_service.py` (923行) | 职责过载，需拆分为 3-4 个子服务 | 2-3 天 |
| P2 | `document_editor_mixin.py` (592行) | utils→features 反向依赖 | 1-2 天 |
| P3 | `matrix_service.py` (243行) | 兼容性转发器需制定清除计划 | 0.5 天 |
| P4 | `report_wizard` → `matrix` 耦合 | 5+ 处直接引用，建议引入 Protocol | 1-2 天 |
| P5 | `src/` 多个文件 (13个) | ~40+ 处残留 print 语句（ltr_number_generator 13处最多） | < 1 天 |
