...
class LTRApplicationFormValidator:
    ...
    def _validate_docx_application_form(self, doc_filepath: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # extracted_version will hold the last found version, or None if not found at all.

            # Final logging based on the results
            if is_valid:
                logger.info(
                    f"Document: {os.path.basename(doc_filepath)} - Validated as application form (Header OK in first section). Version found: {version_number}")
            else:
                logger.info(
                    f"Document: {os.path.basename(doc_filepath)} - NOT validated as application form (Header missing in first section). Version found (if any): {version_number}")
