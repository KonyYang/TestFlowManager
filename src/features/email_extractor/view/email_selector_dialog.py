...
class EmailSelectorDialog(QDialog):
    ...
    def _find_word_attachment(self):
        """查找Word附件"""
    ...
    def _process_word_attachment(self, word_attachment):
        """处理Word附件，检查是否为LTR申请单"""
        try:
            import tempfile
            import os
            ...
            if content:
                ...
                if result and not result.get("error"):
                    # 成功提取数据，打开LTR申请窗口并加载数据
                    self._show_ltr_application_dialog(result)
                else:
                    # 未能提取数据，询问用户是否直接申请LTR编号
                    reply = QMessageBox.question(
                        self,
                        "确认",
                        "没有找到有效的申请单信息，是否直接申请LTR编号？",
                ...
                QMessageBox.warning(self, "警告", "附件内容为空")
        except Exception as e:
            logger.error(f"处理Word附件时出错: {e}")
            QMessageBox.warning(self, "警告", f"处理Word附件失败: {str(e)}")
    ...
    def _show_ltr_application_dialog(self, application_data):
        ...
        try:
            ...
            # 创建LTR编辑对话框
            dialog = LTREditorDialog(dl_data, self.parent())
            ...
            QMessageBox.warning(self, "警告", f"无法打开LTR申请窗口: {str(e)}")
