...
class MatrixSessionEntryFacade:
    ...
    PREVIEW_ISOLATED_PILOT_GATE = "TFM_MATRIX_SESSION_ISOLATED_PREVIEW_PILOT"
    _TRUTHY_SWITCH_VALUES = frozenset({"1", "true", "yes", "on"})

    PREVIEW_PILOT_SESSION_ID = "pilot:preview"
    ...
    @classmethod
    def is_preview_pilot_enabled(cls, env: Mapping[str, str] | None = None) -> bool:
        env_map = env or {}
        value = env_map.get(cls.PREVIEW_ISOLATED_PILOT_GATE, "")
        return value.strip().lower() in cls._TRUTHY_SWITCH_VALUES
    ...
    def resolve_preview_pilot_session_id(self, *, pilot_enabled: bool) -> str | None:
        if not pilot_enabled:
            return None
        return self.PREVIEW_PILOT_SESSION_ID
