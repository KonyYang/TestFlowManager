...
class CRExportService(BaseExportService):
    """CR导出服务"""
    ...
    def export_to_excel(self, file_path, sample_count=5, point_array=None, is_delta_r_checked=False, cr_current_value=""):
        """导出CR到Excel"""
        logger.debug(f"开始导出CR到 {file_path}")
        try:
            ...
            logger.error(f"导出CR失败: {e}", exc_info=True)
            return False
    ...
    def _extract_cr_current_values(self):
        """从Matrix数据中提取CR电流值"""
        cr_values = []
        rows = self.data_model.rows
        ...
        # 查找包含CR的行并提取电流值
        for row in rows:
            if len(row) >= 4 and row[0] and "CR" in str(row[0]).upper():
                condition = row[3]  # Condition列
                # 使用正则表达式提取电流值，如100mA, 1A等
                match = re.search(r'(\d+(?:\.\d+)?)\s*(mA|A)', str(condition))
                if match:
                    value = match.group(1)
                    unit = match.group(2)
                    # 转换为A单位
                    if unit == "mA":
                        cr_values.append(str(float(value) / 1000))
                    else:
                        cr_values.append(value)
        
        return cr_values
    
    def _insert_record_data_table(self, ws, sample_count, point_array, is_delta_r_checked, cr_current_value=""):
        """插入记录数据表格"""
        try:
            # 初始化变量
            headers_cols = 3
            ...
            # 填写原始记录组别和步骤列表头
            ws.cell(row=record_data_tbl_title_row, column=1).value = f"CR {cr_current_value}A" if cr_current_value else "CR"
    ...
    def _apply_formatting(self, worksheet, title_row, sample_count, point_count, 
        ...
        try:
            ...
            logger.debug("CR格式化应用完成")
            
        except Exception as e:
            logger.error(f"应用CR格式化时出错: {e}", exc_info=True)