...
class TableManager:
    ...
    def update_table(self):
        ...
        # 断开信号连接以避免在更新过程中触发事件
        try:
            self.view.matrix_table_widget.itemChanged.disconnect(self.view._on_item_changed)
        except TypeError:
            # 如果信号尚未连接，则忽略错误
            pass
        ...
        try:
            # 清空现有内容
            self.view.matrix_table_widget.clear()
            
            # 设置表头
            self.view.matrix_table_widget.setColumnCount(len(self.controller.data_model.headers))
            self.view.matrix_table_widget.setHorizontalHeaderLabels(self.controller.data_model.headers)
            
            # 设置行数和数据
            self.view.matrix_table_widget.setRowCount(len(self.controller.data_model.rows))
            for row_idx, row_data in enumerate(self.controller.data_model.rows):
                for col_idx, cell_value in enumerate(row_data):
                    if col_idx < len(row_data):  # 确保不越界
                        item = QTableWidgetItem(str(cell_value))
                        self.view.matrix_table_widget.setItem(row_idx, col_idx, item)
            
            # 应用合并单元格（如果有）
            self.apply_merged_cells()
        finally:
            # 重新连接信号
            self.view.matrix_table_widget.itemChanged.connect(self.view._on_item_changed)
    ...
    def apply_merged_cells(self):
        ...
        for row in range(self.view.matrix_table_widget.rowCount()):
            for col in range(self.view.matrix_table_widget.columnCount()):
                self.view.matrix_table_widget.setSpan(row, col, 1, 1)
        ...
        for merge_info in self.controller.data_model.merged_cells_info:
            ...
            # 检查边界
            if (top_row + row_count <= self.view.matrix_table_widget.rowCount() and 
                left_col + col_count <= self.view.matrix_table_widget.columnCount()):
                self.view.matrix_table_widget.setSpan(top_row, left_col, row_count, col_count)
