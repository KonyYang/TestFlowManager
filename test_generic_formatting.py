...
def test_generic_formatting():
    """测试通用Excel格式化服务"""
    print("开始测试通用Excel格式化服务...")
    ...
    # 保存文件
    output_path = "test_generic_formatted_output.xlsx"
    wb.save(output_path)
    print(f"通用格式化后的Excel文件已保存到: {output_path}")
    ...
    # 应用自定义列宽格式化
    print("开始应用自定义列宽格式化...")
    custom_widths = {1: 15, 2: 8, 3: 25}
    formatter.format_worksheet_with_custom_widths(ws2, custom_widths)
    
    # 保存文件
    output_path2 = "test_custom_width_output.xlsx"
    wb2.save(output_path2)
    print(f"自定义列宽格式化后的Excel文件已保存到: {output_path2}")
